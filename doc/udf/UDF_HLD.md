...
- [9. Restrictions/Limitations](#9-restrictionslimitations)
  - [9.1 Configuration Limits](#91-configuration-limits)
  - [9.2 Validation Requirements](#92-validation-requirements)
...
```json
{
  ...
  "UDF_SELECTOR":   { "BTH_RESERVED|roce_v1": { "match": { "l2_type": "0x8915" }, "select": { "base": "L2", "offset": "18" } } },
...
"PACKET":         "[ Ethernet | IP | UDP | RoCE BTH ]  →  byte @ L2+18",
```
...
| Packet type filtering | Field extraction must be conditional on packet type — at minimum L2 EtherType, L3 protocol number, and GRE type, each with an optional mask and priority |
| Flexible extraction point | Extraction offset must be specifiable relative to L2, L3, or L4 header base, with byte offset 0-255 |
| Multi-protocol coverage | A single named field must support extraction from multiple packet types (e.g., RoCE v1 and v2) without requiring separate ACL rules per protocol variant |
...
### 5.2 UDF Object Model(SAI object model)
...
|--------------|----------------|-------------------|
| **UDF** | field_type (GENERIC/HASH), length (1-20) | SAI UDF Group |
| **UDF_SELECTOR** | **match**: L2/L3/GRE type+mask, priority<br/>**select**: base (L2/L3/L4), offset | SAI UDF Match + SAI UDF |

### 5.3 CONFIG_DB to SAI Object Mapping
...
UDF_SELECTOR["BTH_RESERVED|roce_v1"]
  match                       ──→ SAI_OBJECT_TYPE_UDF_MATCH
  select                      ──→ SAI_OBJECT_TYPE_UDF
────────────────────────────────────────────────────────
```

### 5.4 ACL Integration Dependency

```
UDF (udf_field_name: "BTH_RESERVED")
    │
    ├─→ UDF_SELECTOR (key: "BTH_RESERVED|roce_v1", match: {l2_type, priority}, select: {base, offset})
    │
...
### 5.6 Configuration and Data Flow
...
sequenceDiagram
    ...
    Config->>UdfOrch: UDF_SELECTOR config (key: BTH_RESERVED|roce_v1, match={l2_type=0x8915, priority=100}, select={base=L2, offset=18})
    UdfOrch->>SAI: create_udf_match(l2_type=0x8915, priority=100)
...
| **UDF_SELECTOR** | `<udf_field_name>\|<selector_name>` | **match** subgroup:<br/>`l2_type`, `l2_type_mask`<br/>`l3_type`, `l3_type_mask`<br/>`gre_type`, `gre_type_mask`<br/>`priority` (0-255, default 0)<br/>**select** subgroup:<br/>`base` (L2/L3/L4)<br/>`offset` (0-255)<br/>**Mask defaulting**: if a type field is set but mask is omitted, defaults to exact-match (`0xFFFF`/`0xFF`) | Key: `BTH_RESERVED\|roce_v1`<br/>`match.l2_type="0x8915"`<br/>`match.l2_type_mask="0xFFFF"`<br/>`match.priority="100"`<br/>`select.base="L2"`<br/>`select.offset="18"` |
...
```json
{
  ...
  "UDF_SELECTOR": {
    "BTH_RESERVED|roce_v1": {"match": {"l2_type": "0x8915", "l2_type_mask": "0xFFFF", "priority": "100"}, "select": {"base": "L2", "offset": "18"}},
...
**Scenario**: Match RoCE v1 packets where the BTH Reserved field (L2 offset 18, 1 byte) is non-zero — i.e., drop packets that violate the protocol requirement that the reserved field must be zero.
...
**Step 3: Create SAI UDF (L2 base, offset 18 — BTH Reserved byte)**
```c
...
udf_attrs[3].value.u16 = 18;  // BTH Reserved byte at L2+18
...
| **UDF_SELECTOR** | **match**: `l2_type`/`l3_type`/`gre_type` (hex string), masks, `priority` (0-255)<br/>**select**: `base` (enum: L2/L3/L4), `offset` (0-255) | `select.base` and `select.offset` mandatory; at least one `match` type field required; parent UDF entry must exist |
...
| **UDF_SELECTOR match criteria** | `match` subgroup: at least one of `l2_type`, `l3_type`, or `gre_type` must be non-zero (checked against both data and mask) | Reject config, log error |
| **UDF_SELECTOR select fields** | `select` subgroup: `base` and `offset` are mandatory | Reject config, log error |
| **UDF_SELECTOR mask defaulting** | If a type field in `match` is set but mask is omitted, mask defaults to exact-match (0xFF / 0xFFFF). Applied in `doUdfSelectorTask` after the field parse loop. | Silent default, logged at debug level |
| **udf_field deletion** | Blocked if ACL table ref count > 0 (tracked via `incrementGroupRefCount` / `decrementGroupRefCount`) | Reject deletion, return false (triggers retry) |
| **UDF_SELECTOR deletion** | Blocked if ACL rule ref count > 0 and this is the last selector for the field (tracked via `incrementUdfRuleRefCount` / `decrementUdfRuleRefCount`) | Reject deletion, return false (triggers retry) |
| **UDF_SELECTOR dependencies** | `doUdfSelectorTask` retries until the parent udf_field OID is available | Retry until dependency resolved |
| **Config immutability** | If a UDF/UDF_SELECTOR already exists with different attributes, the update is rejected — SAI UDF objects are CREATE_ONLY; delete and recreate to change config | Reject config, log error |
| **ACL_TABLE_TYPE** | `matches` field must reference existing udf_field names | Reject config, log error |
| **ACL_RULE udf_field reference** | udf_field name in ACL_RULE (e.g., `BTH_RESERVED`) must be declared in the ACL_TABLE_TYPE `matches` field | Reject config, log error |
| **One udf_field per rule** | A single ACL_RULE references each udf_field at most once (a udf_field produces one value per packet) | Reject config, log error |
### 9.3 Platform and Feature Limitations
...
| **No reassignment** | SAI UDF group/match/udf attributes are CREATE_ONLY; delete and recreate to change |
...
```json
{
  ...
  "UDF_SELECTOR": {
    "BTH_RESERVED|roce_v1": {"match": {"l2_type": "0x8915", "l2_type_mask": "0xFFFF", "priority": "100"}, "select": {"base": "L2", "offset": "18"}},
    "BTH_RESERVED|roce_v2": {"match": {"l3_type": "0x11",   "l3_type_mask": "0xFF",   "priority": "100"}, "select": {"base": "L4", "offset": "8"}}
  },
  "ACL_TABLE_TYPE": {
