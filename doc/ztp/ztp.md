...
The active HWSKU is persisted to */host/ztp/ztp_hwsku* on every HWSKU load. On resume (e.g. after a ZTP-initiated firmware upgrade), ZTP reads this cache and reloads the correct HWSKU before restarting DHCP discovery. This ensures that a firmware upgrade that is launched from a non-default hwsku does not have to go through the cycling process again. See section 3.8 (Image Upgrade) for details.
...
- Ensure that the new firmware keeps the same default HWSKU
